...
impl CompilationDB {
    pub fn new_fs(
        ...
        param_defaults: &[CliParamDefault],
    ) -> Result<Self> {
    ...
    pub fn new<'a>(
        ...
        param_defaults: impl Iterator<Item = &'a CliParamDefault>, // &[CliParamDefault],
    ) -> Result<Self> {
        ...
        println!("Root file id is {:?}", root_file);
        ...
        let defaults: Arc<[_]> = param_defaults.cloned().collect();
        res.set_cli_param_defaults(root_file, defaults);
        Ok(res)
    }
}
